...
| Outbox-style staging for the Execution Dispatcher (vs. direct-produce, 2.2's exception / 3.5) | Direct-produce leaves the Dispatcher's publish dependent on Kafka being available at that moment, rather than decoupled from it the way Outbox decouples every other write path; doing this for every due plan (hundreds of thousands/day) at current scale would add a DB write and a Debezium hop with no operational problem yet observed to justify it. |
